...
unsigned long tempoMedicaoAnterior = 0;
const int intervaloAmostragem = 100; // Intervalo de 100 ms para cada medição. Ajuste se necessário.
...
void setup() {
  ...
  Serial.println("LABEL,Tempo (s),PWM, PWM (%),Empuxo (g),Corrente (mA),Tensao (V),Potencia (mW),Velocidade (RPM)");
...
void loop() {
  ...
  if (FINISH == false){
    PWM_input = (20*(millis()-inicioaquisicao)/1000) + 1012; // Ajustar de acordo com a velocidade da rampa de PWM
    if (PWM_input > 2020){
      PWM_input = 2020;
    }
    if (PWM_input < 1012){
  ...
  if (FINISH && millis() - TempoPWMmaximo > 4000){
    ESC.write(1012); // 1012
  }

  if (PWM_input >= 2020 && FINISH == false){
    TempoPWMmaximo = millis();
    FINISH = true;
  }

   

   // Verifica se já passou o tempo para uma nova medição de RPM
  if (millis() - tempoMedicaoAnterior >= intervaloAmostragem) {

    tempoMedicaoAnterior = millis();
  ...
  if (millis() - TempoAnterior >= IntervaloLeitura){
    ...
    PWM_percentage = map(PWM_input,1012,2020,0,100);
    Serial.print("DATA,");
    ...
    Serial.print(scale.get_units(3), 1);                 Serial.print(",");
